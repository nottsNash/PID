...
volatile long count_encoder_right; // used by encoder to count the rotation (0)
volatile byte state_encoder_right;
volatile long count_encoder_left; //(1)
volatile byte state_encoder_left;



// This ISR handles just Encoder 0 (right)
...
ISR( INT6_vect ) {
  ...
  // State:  new B   new A   old B   old A
    state_encoder_right = state_encoder_right | ( e0_B  << 3 );
    state_encoder_right = state_encoder_right | ( e0_A  << 2 );

    // Handle which transition we have registered.
    // Complete this if statement as necessary.
    // Refer to the labsheet. 
    if( state_encoder_right == 0 ) {
      //no change
    } else if( state_encoder_right == 1 ) {
      count_encoder_right = count_encoder_right + 1;
      
    } else if( state_encoder_right == 2 ) {
      count_encoder_right = count_encoder_right - 1;
      
    } else if( state_encoder_right == 4)  {
      count_encoder_right = count_encoder_right - 1;
      
    } else if( state_encoder_right == 7 ) {
      count_encoder_right = count_encoder_right + 1;
      
    } else if( state_encoder_right == 8 ) {
      count_encoder_right = count_encoder_right + 1;
      
    } else if( state_encoder_right == 11 ) {
      count_encoder_right = count_encoder_right - 1;
      
    } else if( state_encoder_right == 13)  {
      count_encoder_right = count_encoder_right - 1;
      
    } else if( state_encoder_right == 14 ) {
      count_encoder_right = count_encoder_right + 1;
    }
    ...
    state_encoder_right = state_encoder_right >> 2;

}


// This ISR handles just Encoder 1 (left)
...
ISR( PCINT0_vect ) {
    ...
    state_encoder_left = state_encoder_left | ( e1_B  << 3 );
    state_encoder_left = state_encoder_left | ( e1_A  << 2 );


    // Handle which transition we have registered.
    // Complete this if statement as necessary.
    // Refer to the labsheet. 
    if( state_encoder_left == 0 ) {
      //no change
    } else if( state_encoder_left == 1 ) {
      count_encoder_left = count_encoder_left + 1;
      
    } else if( state_encoder_left == 2 ) {
      count_encoder_left = count_encoder_left - 1;
      
    } else if( state_encoder_left == 4)  {
      count_encoder_left = count_encoder_left - 1;
      
    } else if( state_encoder_left == 7 ) {
      count_encoder_left = count_encoder_left + 1;
      
    } else if( state_encoder_left == 8 ) {
      count_encoder_left = count_encoder_left + 1;
      
    } else if( state_encoder_left == 11 ) {
      count_encoder_left = count_encoder_left - 1;
      
    } else if( state_encoder_left == 13)  {
      count_encoder_left = count_encoder_left - 1;
      
    } else if( state_encoder_left == 14 ) {
      count_encoder_left = count_encoder_left + 1;
    }
    // Shift the current readings (bits 3 and 2) down
    // into position 1 and 0 (to become prior readings)
    // This bumps bits 1 and 0 off to the right, "deleting"
    // them for the next ISR call. 
    state_encoder_left = state_encoder_left >> 2;
}
...
void setupEncoder0() 
{
    ...
    state_encoder_right = state_encoder_right | ( e0_B << 1 );
    state_encoder_right = state_encoder_right | ( e0_B << 0 );
